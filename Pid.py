import numpy


class Pid:
    def __init__(self, Kp_, Ki_, Kd_, dt_ = 1.):
        self._Kp_ = Kp_
        self._Ki_ = Ki_
        self._Kd_ = Kd_
        self._dt_ = dt_
        self._P_ = 0
        self._I_ = 0
        self._D_ = 0
        self._integral = 0.
        self._T0_ = None
        self._T1_ = None
        self._e0_ = None
        self._e1_ = None

    def __call__(self, setPoint, T_):
        output = 0.
        
        self._T0_ = T_
        self._e0_ = setPoint - self._T0_

        if self._T1_ is not None:
            if (numpy.abs(self._e0_) > 2.):
                self._integral = 0
            else:
                self._integral += self._e0_ * self._dt_
            
            dedt_ = (self._e0_ - self._e1_) / self._dt_

            self._P_ = self._e0_ * self._Kp_
            self._I_ = self._integral * self._Ki_
            self._D_ = dedt_ * self._Kd_

            output = self._P_ + self._I_ + self._D_

        self._T1_ = self._T0_
        self._e1_ = self._e0_

        return output

    @property
    def Kp(self):
        return self._Kp_

    @property
    def Ki(self):
        return self._Ki_

    @property
    def Kd(self):
        return self._Kd_

    @property
    def PID(self):
        return [self._P_, self._I_, self._D_]
